...
const MainContent = ({ onTrackSelect, selectedPlaylist }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // FIX: Ensure we call the correct endpoint for filtered tracks
        ...
        // If the playlist is empty, the backend should return [], not all tracks
        ...
        setTracks([]); // Reset on error to prevent showing global library inside a playlist
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRemoveFromPlaylist = async (trackId) => {
    try {
      // FIX: Ensure the route matches the backend DELETE route
      ...
      alert("Failed to remove song. Check backend routes.");
    }
  };
    ...
    <div className="p-8 pb-32 h-full overflow-y-auto bg-black">
      ...
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-3xl font-bold text-white">
          {selectedPlaylist ? `Playlist: ${selectedPlaylist.name}` : "Explore Music"}
        </h2>
        ...
        {filteredTracks.map((track) => (
          <div key={track.id} className="bg-zinc-900/40 p-4 rounded-xl hover:bg-zinc-800/60 transition-all group relative">
            <div className="relative aspect-square mb-4 shadow-lg overflow-hidden rounded-lg" onClick={() => onTrackSelect(track)}>
              <img src={track.cover_url || track.cover_image} alt={track.title} className="object-cover w-full h-full group-hover:scale-105 transition-transform" />
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="relative">
                {selectedPlaylist ? (
                  // Show Trash icon only inside specific playlists
                  <button onClick={() => handleRemoveFromPlaylist(track.id)} className="text-zinc-500 hover:text-red-500">
                    <Trash2 size={18} />
                  </button>
                ) : (
                  <>
                    <button onClick={() => setActiveMenu(activeMenu === track.id ? null : track.id)} className="text-zinc-500 hover:text-green-500 transition-all">
                      <Plus size={18} />
                    </button>
